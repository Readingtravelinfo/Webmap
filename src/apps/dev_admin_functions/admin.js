...
function pageSetup2(){
	...
	var fH, fF, blankA, blankA2, e, textStr, single;
	...
	if (usertext==="") {
		...
		if (okint == 0) {
			...
			if (eC.configuredMap===''){
				...
				for (var key in tmp1){
					if(key==fieldL[0]){
						//Assuming set the fieldL option to the first record!
						single = 1;
					}
					if(single===1){
						//Only one record
						if(key!=='remove'){
							oObj1[key].push(tmp1[key]);
						}
					} else {
						//We need to loop through a second object
						if(key!=='remove'){
				...
				element = 'exclusions';
				arrayN = 'pExclusionA';
				blankA = 'exclusion'; //This field defines whether this is a valid record or not
				//Logic to check for single value or array
				for(popi=0;popi<fieldL.length;popi++){
					window[arrayN][1].push([]);
					tmpS = fieldL[popi];
				...
				element = 'disable';
				arrayN = 'pDiscolA';
				blankA = 'optionT'; //This field defines whether this is a valid record or not
				//Logic to check for single value or array
				for(popi=0;popi<fieldL.length;popi++){
					window[arrayN][1].push([]);
					tmpS = fieldL[popi];
				...
				arrayN = 'pStyleA';
				blankA = 'styleLayer'; //This field defines whether this is a valid record or not
				//Logic to check for single value or array
				for(popi=0;popi<fieldL.length;popi++){
					window[arrayN][1].push([]);
					tmpS = fieldL[popi];
...
function moveRow(dir, tdID, rowID){
	...
	//OK, if this was the layer list then we need to update the next three tables
	if(tdID==="pLayer"){
		rowID = "r" + rowID + "c";
		moveRow(dir, "pPopHov", rowID);
		moveRow(dir, "pZoom", rowID);
		moveRow(dir, "pWFS", rowID);
	}
}

/*This function allows us to remove a row from a table*/
function deleteRow(tdID, rowID, tabID){
	var tdIDsave = tdID;
	tdID = '#' + tdID;
	rowID = rowID.replace("r","").replace("c","");
	rowID = parseInt(rowID);
	...
	//We need to adjust the id values of other table rows
	delID = new RegExp('r' + rowID + 'c','g');
	
	//Pickup the table length
	var tbLen = $('#' + tabID + ' tr').length;
	var nr, jrefID;
	if (rowID<tbLen){
		//We need to update the rows below the deleted row
		var currRow = rowID;
		//Replace the ID's other rows
		for (nr=currRow;nr<tbLen;nr++){
			tmpI2 = nr + 1;
			jrefID = nr + 1;
			tmpID = new RegExp('r' + tmpI2 + 'c','g');
			tmpID2 = 'r' + (tmpI2-1) + 'c';
			tmpTR = $('#' + tabID).find('tr:nth-child('+(jrefID)+')'); //Pickup the row to modify
			tmpHTML = tmpTR.html();
			tmpHTML = tmpHTML.replace(tmpID,tmpID2);
			tmpTR.html(tmpHTML);
		}
	} //If this is the last record we don't need to do anything at all
	
	//OK, if this was the layer list then we need to update the next three tables
	if(tabID==="pLayer"){
		rowID = "r" + rowID + "c";
		deleteRow(tdIDsave.replace('pL','pUh'), rowID, "pPopHov");
		deleteRow(tdIDsave.replace('pL','pZ'), rowID, "pZoom");
		deleteRow(tdIDsave.replace('pL','pWFS'), rowID, "pWFS");
	}
}
					...
					$('#pStyle tr').each(function() {
						if (l5!==0){
							...
							for(l4=0;l4<($('#pLayer tr').length-1);l4++){
								...
								layerSelHTML += '<option value="overlay' + l4 + '">' + tmpVal.replace(/&/g,'&amp;') + '</option>';
							}
							layerSelHTML += '</select>';
...
function saveConfig(){
	...
	tmpArr.each(function() {
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				tmpVal = $(this).text();
				oXML += '    <option>' + tmpVal.replace(/&/g,'&amp;') + '</option>\n';
			}
		})
	});
	oXML += '  </userArray>\n';
	...
	tmpVal = $('#mP').find(":selected").val();
	if(tmpVal.indexOf('EPSG:')!==-1){
		oXML += '      <projMap>' + tmpVal.replace(/&/g,'&amp;') + '</projMap>\n';
	} else {
		oXML += '      <projMap>EPSG:' + tmpVal.replace(/&/g,'&amp;') + '</projMap>\n';
	}
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '      <mapURL>' + tmpVal.replace(/&/g,'&amp;') + '</mapURL>\n';
					...
					oXML += '      <mapTitle>' + tmpVal.replace(/&/g,'&amp;') + '</mapTitle>\n';
				}
			}
		})
	});
			...
			$('td', this).each(function(){
				...
				if (typeof tmpID !=='undefined'){
					...
					if (tmpID.indexOf("c3")!==-1) {
						...
						oXML += '      <overlayTitle>' + tmpVal.replace(/&/g,'&amp;') + '</overlayTitle>\n';
						...
						oXML += '      <overlayDILS>' + tmpVal.replace(/&/g,'&amp;') + '</overlayDILS>\n';
						...
						oXML += '      <onasdefault>' + tmpVal.replace(/&/g,'&amp;') + '</onasdefault>\n';
						...
						if(tmpVal.indexOf('EPSG:')!==-1){
							oXML += '      <overlaySRS>' + tmpVal.replace(/&/g,'&amp;') + '</overlaySRS>\n';
						} else {
							oXML += '      <overlaySRS>EPSG:' + tmpVal.replace(/&/g,'&amp;') + '</overlaySRS>\n';
						}
						...
						oXML += '      <overlayDOME>' + tmpVal.replace(/&/g,'&amp;') + '</overlayDOME>\n';
						...
						oXML += '      <overlayDDtitle>' + tmpVal.replace(/&/g,'&amp;') + '</overlayDDtitle>\n';
						...
						oXML += '      <overlaySTYLES>' + tmpVal.replace(/&/g,'&amp;') + '</overlaySTYLES>\n';
						...
						oXML += '      <overlayCache>' + tmpVal.replace(/&/g,'&amp;') + '</overlayCache>\n';
						...
						oXML += '      <overlayTRAN>' + tmpVal.replace(/&/g,'&amp;') + '</overlayTRAN>\n';
						...
						oXML += '      <overlayENV>' + tmpVal.replace(/&/g,'&amp;') + '</overlayENV>\n';
						...
						oXML += '      <overlayAddress>' + tmpVal.replace(/&/g,'&amp;') + '</overlayAddress>\n';
					} else if (tmpID.indexOf("c2")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayPath>' + tmpVal.replace(/&/g,'&amp;') + '</overlayPath>\n';
					} 
				}
			})
			...
			$('td', this).each(function(){
				...
				if (typeof tmpID !=='undefined'){
					if (tmpID.indexOf("c2")!==-1){
						...
						oXML += '      <overlayPopup>' + tmpVal.replace(/&/g,'&amp;') + '</overlayPopup>\n';
						...
						oXML += '      <overlayPopupTemp>' + tmpVal.replace(/&/g,'&amp;') + '</overlayPopupTemp>\n';
					} else if (tmpID.indexOf("c4")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayPopupWidth>' + tmpVal.replace(/&/g,'&amp;') + '</overlayPopupWidth>\n';
					} else if (tmpID.indexOf("c5")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayPopupHeight>' + tmpVal.replace(/&/g,'&amp;') + '</overlayPopupHeight>\n';
					} else if (tmpID.indexOf("c6")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayHoverTemp>' + tmpVal.replace(/&/g,'&amp;') + '</overlayHoverTemp>\n';
					} else if (tmpID.indexOf("c7")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayHoverWidth>' + tmpVal.replace(/&/g,'&amp;') + '</overlayHoverWidth>\n';
					} else if (tmpID.indexOf("c8")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayHoverHeight>' + tmpVal.replace(/&/g,'&amp;') + '</overlayHoverHeight>\n';
					} 
				}
			})
			oXML += '    </overlay2>\n';
			...
			$('td', this).each(function(){
				...
				if (typeof tmpID !=='undefined'){
					if (tmpID.indexOf("c2")!==-1){
						...
						oXML += '      <overlayZoom>' + tmpVal.replace(/&/g,'&amp;') + '</overlayZoom>\n';
					} else if (tmpID.indexOf("c3")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayZoomLevel>' + tmpVal.replace(/&/g,'&amp;') + '</overlayZoomLevel>\n';
					} else if (tmpID.indexOf("c4")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayZoomSelF>' + tmpVal.replace(/&/g,'&amp;') + '</overlayZoomSelF>\n';
					} else if (tmpID.indexOf("c5")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayZoomRepT>' + tmpVal.replace(/&/g,'&amp;') + '</overlayZoomRepT>\n';
					} else if (tmpID.indexOf("c6")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayZoomRepF>' + tmpVal.replace(/&/g,'&amp;') + '</overlayZoomRepF>\n';
					} 
				}
			})
			oXML += '    </overlay3>\n';
			...
			$('td', this).each(function(){
				...
				if (typeof tmpID !=='undefined'){
					if (tmpID.indexOf("c2")!==-1){
						...
						oXML += '      <overlayType>' + tmpVal.replace(/&/g,'&amp;') + '</overlayType>\n';
					} else if (tmpID.indexOf("c3")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayTable>' + tmpVal.replace(/&/g,'&amp;') + '</overlayTable>\n';
					} else if (tmpID.indexOf("c4")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayfeatureType>' + tmpVal.replace(/&/g,'&amp;') + '</overlayfeatureType>\n';
					} else if (tmpID.indexOf("c5")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlayfeatureNS>' + tmpVal.replace(/&/g,'&amp;') + '</overlayfeatureNS>\n';
					} else if (tmpID.indexOf("c9")!==-1) {
						tmpVal = $(this).html();
						if(tmpVal.indexOf('select')!==-1){
							//This is a select option, get the value
						...
						oXML += '      <overlaygeometryName>' + tmpVal.replace(/&/g,'&amp;') + '</overlaygeometryName>\n';
					} 
				}
			})
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '      <styleLayer>' + tmpVal.replace(/&/g,'&amp;') + '</styleLayer>\n';
				} else if (tmpID.indexOf("c2")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <styleTitle>' + tmpVal.replace(/&/g,'&amp;') + '</styleTitle>\n';
				} 
			}
		})
	});
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '      <targetCol>' + tmpVal.replace(/&/g,'&amp;') + '</targetCol>\n';
				} else if (tmpID.indexOf("c2")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <targetTable>' + tmpVal.replace(/&/g,'&amp;') + '</targetTable>\n';
				} else if (tmpID.indexOf("c3")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <replaceCol>' + tmpVal.replace(/&/g,'&amp;') + '</replaceCol>\n';
				} else if (tmpID.indexOf("c4")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <replaceTable>' + tmpVal.replace(/&/g,'&amp;') + '</replaceTable>\n';
				} 
			}
		})
	});
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '        <optionsListOp>' + tmpVal.replace(/&/g,'&amp;') + '</optionsListOp>\n';
				} else if (tmpID.indexOf("c2")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '        <optionListNo>' + tmpVal.replace(/&/g,'&amp;') + '</optionListNo>\n';
				}
			}
		})
	});
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c3")!==-1){
					...
					if (tmpVal!==''){
						oXML += '        <optionsListCol>' + tmpVal.replace(/&/g,'&amp;') + '</optionsListCol>\n';
					}
				} else if (tmpID.indexOf("c4")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
					...
					if (tmpVal!==''){
						oXML += '        <optionsListColLoop>' + tmpVal.replace(/&/g,'&amp;') + '</optionsListColLoop>\n';
					}
				}
			}
		})
	...
	oXML += '  </optionsList>\n';
	oXML += '  <status>\n';
	//Pickup the number of rows in the user table //.replace(/&/g,'&amp;') to deal with illegal XML character &
	tmpArr = $('#pStatus tr');
	tmpNo = tmpArr.length;
	tmpArr.each(function() {
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '      <statusValue>' + tmpVal.replace(/&/g,'&amp;') + '</statusValue>\n';
				} else if (tmpID.indexOf("c2")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <statusAvail>' + tmpVal.replace(/&/g,'&amp;') + '</statusAvail>\n';
				} else if (tmpID.indexOf("c3")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <statusText>' + tmpVal.replace(/&/g,'&amp;') + '</statusText>\n';
				} else if (tmpID.indexOf("c4")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <statusTable>' + tmpVal.replace(/&/g,'&amp;') + '</statusTable>\n';
				} else if (tmpID.indexOf("c5")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <statusField>' + tmpVal.replace(/&/g,'&amp;') + '</statusField>\n';
				} 
			}
		})
	});
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '      <exclusion>' + tmpVal.replace(/&/g,'&amp;') + '</exclusion>\n';
				} else if (tmpID.indexOf("c2")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <exclusionT>' + tmpVal.replace(/&/g,'&amp;') + '</exclusionT>\n';
				} 
			}
		})
	});
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '      <optionT>' + tmpVal.replace(/&/g,'&amp;') + '</optionT>\n';
				} 
			}
		})
	});
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '      <tableName>' + tmpVal.replace(/&/g,'&amp;') + '</tableName>\n';
				} else if (tmpID.indexOf("c2")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <tableCondition>' + tmpVal.replace(/&/g,'&amp;') + '</tableCondition>\n';
				} 
			}
		})
	});
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '      <tableGeom>' + tmpVal.replace(/&/g,'&amp;') + '</tableGeom>\n';
				} else if (tmpID.indexOf("c3")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <tableTitle>' + tmpVal.replace(/&/g,'&amp;') + '</tableTitle>\n';
				} else if (tmpID.indexOf("c4")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <selStyle>' + tmpVal.replace(/&/g,'&amp;') + '</selStyle>\n';
				} else if (tmpID.indexOf("c5")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <recNo>' + tmpVal.replace(/&/g,'&amp;') + '</recNo>\n';
				} else if (tmpID.indexOf("c6")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <lower>' + tmpVal.replace(/&/g,'&amp;') + '</lower>\n';
				} else if (tmpID.indexOf("c7")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <order_by>' + tmpVal.replace(/&/g,'&amp;') + '</order_by>\n';
				} else if (tmpID.indexOf("c8")!==-1){
					tmpVal = $(this).html();
					if(tmpVal.indexOf('select')!==-1){
						//This is a select option, get the value
					...
					oXML += '      <geom_field>' + tmpVal.replace(/&/g,'&amp;') + '</geom_field>\n';
				} 
			}
		})
	});
		...
		$('td', this).each(function(){
			...
			if (typeof tmpID !=='undefined'){
				if (tmpID.indexOf("c1")!==-1){
					...
					oXML += '      <functionEx>' + tmpVal.replace(/&/g,'&amp;') + '</functionEx>\n';
				} 
			}
		})
	});
