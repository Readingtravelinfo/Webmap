//Global variables
var i, i2, centrePanel, mapPanel, legendPanel, tableport, viewport, photoscroller, photoscroller2, pslow, pshigh, photoArray, tree, tableURL, accordion, fieldNames, currGID, tableWidth;
var east1, east1b, east2, east3, east4, east5, east6;
var recNo, lower, filter, tempOverlay;
	...
	if (call==1) {
		...
		while (i>=0){
			if (overlayDILS[i]==true){
				...
				var overlayAddress = tmpWFSname3[i];
				...
				for (i3=0;i3<sld.length;i3++){
					...
					styleFilter.push(sld[i3].filter);
					styleText.push(sld[i3].title);
					//NB - spaces at the end of a style name will stop the icons loading!
					styleImage.push(Dpath + 'wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=' + overlayAddress + '&RULE=' + sld[i3].name);
					styleNum.push(i);
		...
		//legendHandler(f1,f2,f3,type)
		var currentOL, Tmpfilter;
		i2 = 0;
		for (i=0;i<styleArray.length;i++){
			if (i==0){
				//First time so set the first layer
				currentOL = styleArray[i];
				rootNode.attributes["children"] = [{text: styleLName[i], "leaf": false, "checked": false, "expanded":true, "layer":currentOL, "ftype":"layerSwitch", "fproperty":"", "fvalue":""}];
				Tmpfilter = styleFilter[i];
				rootNode.attributes.children[i2]["children"] = [{text: styleText[i], "leaf": true, "checked": false, "expanded":false, "icon": styleImage[i],"layer":currentOL, "ftype":Tmpfilter.type, "fproperty":Tmpfilter.property, "fvalue":Tmpfilter.value}];
			} else if (styleArray[i]!=currentOL){
				//This is a new layer
				i2 = i2 + 1;
				currentOL = styleArray[i];
				rootNode.attributes.children.push({text: styleLName[i], "leaf": false, "checked": false, "expanded":true, "layer":currentOL, "ftype":"layerSwitch", "fproperty":"", "fvalue":""});
				Tmpfilter = styleFilter[i];
				rootNode.attributes.children[i2]["children"] = [{text: styleText[i], "leaf": true, "checked": false, "expanded":false, "icon": styleImage[i],"layer":currentOL, "ftype":Tmpfilter.type, "fproperty":Tmpfilter.property, "fvalue":Tmpfilter.value}];
			} else {
				//This is a normal child node
				Tmpfilter = styleFilter[i];
				rootNode.attributes.children[i2].children.push({text: styleText[i], "leaf": true, "checked": false, "expanded":false, "icon": styleImage[i],"layer":currentOL, "ftype":Tmpfilter.type, "fproperty":Tmpfilter.property, "fvalue":Tmpfilter.value});
			}
		}
		jsonStringReady = 1;
	}
...
// //////////////////////////
function dd() {
	var parts, part1, part2, part3, part4, url, objString, layerObj;
	parts = document.getElementById('ddp1').value.split('|');
	part1 = parts[0] + parts[1] + parts[2] + parts[3]
	part2 = document.getElementById('ddp2').value;
	part3 = '';
	part4 = parts[0].replace('/geoserver/','/apps/functions/sirdownloadalot.php');
	document.getElementById('ddupdate').innerHTML = 'Preparing your download...please wait';
	
	layerObj = window[parts[5]];
	//Check for a non-default style
	if (typeof layerObj.styles != 'undefined'){
		part3 = "&styles=" + layerObj.styles;	
		alert(part3);
	}
	...
	if (part1 == 1 || part2 == 1){
		...
		//Repopulate using the outputFormat specified
		if (part2 == '&outputFormat=KML'){
			//Check for a filter
			var CQLoutputFormat = new OpenLayers.Format.CQL();
			var filterActive = 'N';
			if (typeof layerObj.filter != 'undefined'){
				if (layerObj.filter != null){
					CQLoutputFormat = CQLoutputFormat.write(layerObj.filter);
					filterActive = 'Y';
				}
			}
			
			//Generate the URL and post
			part1 = part1.substring(part1.indexOf("&typeName=")+10);  //This gives the layer name
			if (filterActive == 'Y'){
				part3 += "&format_options=kmattr:true;legend:false;kmscore:100;kmltitle:" + parts[4] + "&mode=download&CQL_FILTER=" + CQLoutputFormat.replace(/\*/g,"%"); //Can't add a legend as this would password protect the file
			} else {
				part3 += "&format_options=kmattr:true;legend:false;kmscore:100;kmltitle:" + parts[4] + "&mode=download"; //Can't add a legend as this would password protect the file
			}
			url = Dpath + "wms/kml?layers=" + part1 + part3;
			$.post(url,function(data){
				...
				document.getElementById('sdalval2').value = parts[4].replace(/ /g, "_") + ".kml";
				document.getElementById('sdalval3').value = 'application/vnd.google-earth.kml+xml';
				document.getElementById('ddupdate').innerHTML = '<a href="' + url + '" target="_blank">Link for Manual Download</a>';
			...
			if (typeof layerObj.filter != 'undefined'){
				if (layerObj.filter != null){
					//alert(layerObj.filter.toSource());
				}
			}
			...
			$.post(url,function(data){
				...
				document.getElementById('sdalval').value = objString;
				document.getElementById('sdalval2').value = parts[4].replace(/ /g, "_") + ".csv";
				document.getElementById('sdalval3').value = 'application/octet-stream';
				document.getElementById('ddupdate').innerHTML = '<a href="' + url + '" target="_blank">Link for Manual Download</a>';
				document.getElementById('sirdownloadalotform').submit();
			});
		} else {
			//This catches any filetype that we have not got a post for (ie we cannot filter)
			url = part1 + part2 + part3;
			document.getElementById('ddupdate').innerHTML = '<a href="' + url + '" target="_blank">Link for Manual Download</a>';
...
function zoomConSwitch(){
	//turn on and off the zoomControl action
	if (typeof zoomCon.active == 'null'){
		zoomCon.activate();
	} else {
		if (zoomCon.active==true){
			zoomCon.deactivate();
		} else {
			zoomCon.activate();
		}
	...
	if (usertext==="") {
		...
		if (okint == 0) {
			...
			for (i=0;i<wfsPath.length;i++){
				...
				window[tmpON].events.register("outfeature", this, function(e) {
					report(e);
							...
							getfeatureinfo: function(event) {
								...
								if (typeof event.features[0] != 'undefined'){
									...
									if (doHover[diamOpts]==true){
										tmpHW = overlayHoverWidth[diamOpts];
										tmpHH = overlayHoverHeight[diamOpts];
										
										currentLayerV = "hoverBox" + diamOpts;
										if (!window[currentLayerV]){
											//No hover here
											fixEmpty = 0;
											window[varname2] = 0;
										} else {
											if (window[currentLayerV].dialog("isOpen")==true){
												window[currentLayerV].dialog("close");
											}
											window[varname2] = 0;								   
										}
										...
										if (features.length != 0 ) { //Prevent hover where there is no result
											...
											if (tmpHW.indexOf("px")!=-1){
												tmpHW = tmpHW.substr(0,tmpHW.indexOf("px"));
											}
											tmpHW = parseInt(tmpHW);
											if (tmpHH.indexOf("px")!=-1){
												tmpHH = tmpHH.substr(0,tmpHH.indexOf("px"));
											}
											tmpHH = parseInt(tmpHH);
											window[currentLayerV] = $(htmlText).dialog({
												position: [event.xy.x, event.xy.y],
												width: tmpHW,
												maxHeight: tmpHH
											});
										}
									...
									for(i=0;i<wfsFT.length;i++){
										currentLayerV = "hoverBox" + i;
										if (typeof window[currentLayerV] != 'undefined'){
											if (window[currentLayerV].dialog("isOpen")==true){
												window[currentLayerV].dialog("close");
											}
										}
				...
				var tree = new Ext.tree.TreePanel({
					...
					listeners: {
						...
						checkchange: function(node, checked){
							if(checked){
								legendHandler(node, 'checked');
							}else{
								legendHandler(node, 'unchecked');
							}
						}
					},
					lines: false
			...
			if (tableArray.length!=0){
				...
				for (i=0;i<tableArray.length;i++){
					fmhtml += "<option value='~T~" + tableArray[i] + "'>" + tableTitles[i] + "</option>";
				}
			}
			fmhtml += "</select><br />";
			...
			for (i=0;i<overlayAddress.length;i++){
				if (overlayDDtitle[i]!=''){
					...
					ddhtml += "<option value='" + Dpath + "|" + workS + "|" + "/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + "|" + overlayAddress[i] + "|" + overlayDDtitle[i] + "|" + wfsArray[i] + "'>" + overlayDDtitle[i] + "</option>";
				}
			}
...
function selectTog(conOn, groupN){
	...
	if (selectOn == 'Y'){
		...
		if (infoSwitch.length > 0){
			for (i=0;i<wfsArray.length;i++){
				if (window[infoSwitch[i]].active==false){
					window[infoSwitch[i]].activate();
				}
			}
		}
		//Deactivate the active tool
		edtoggleControl2("select");
		...
		for (i=0; i< actionText.length; i++){
			if (actionText[i]!='' && actionText[i]!='-' && actionText[i]!='->'){
				if(actionText[i]==conOn  && actionGroup[i]==groupN){
					...
					//Check for active select tool
					selectOn = 'N';
					for (i2=0;i2<wfsArray.length;i2++){
						var tmpON2 = "sH" + i2;
						if (window[tmpON2].active == true){
							selectOn = 'Y';
						}
					}
					//Switch off the select feature tool if enabled
					if (conOn!='Select a Feature' && selectOn=='Y'){
						selectTog('Select a Feature', groupN);
					} 
...
function updateTable(tableHTML, override, tableSwap){
	...
	if (override != 'override'){
		tableHTML = "../../apps/functions/results.php?tableWidth=" + tableWidth + "&table=" + table + "&function=" + document.getElementById('functionS').value;
