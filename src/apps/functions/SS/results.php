...
/*$fixCol = array();
foreach ($xml->other->fixCol->option as $opt) {
	array_push($fixCol, $opt);
}
$fixColno = count($fixCol);*/
...
if ($filterLen>1) {
    ...
    for ($ot=0;$ot<$opTranslatorno;$ot++){
        if (strstr($filter,$opTranslator[$ot])){
			...
			$filterA = str_split($filter); 
			//The ¥ is used as the deliminator, how many are there?
			$faA = substr_count($filter,'¥');

			if ($faA == 3) {
				//Simple filter will equal 3 ?¥field¥=¥value
				$faA = 1;
			} else {
				//complex filters will have 5 more per extra?¥field¥=¥value¥and/or¥?¥field¥=¥value
				$faA = $faA - 3; //Remove the first one
				$faA = $faA / 5; //Divide by 5 
				$faA = $faA + 1; //Add the default first one back
			}
			
			//These variables create a variable variable later so $q is q10 which references variable $q10
			$faB = 1;
			$faC = 0;
			$q = 'q' . $faB . $faC;
	
			//Lets loop through the string
			/* It is very important to understand this loop works character by character and that the ¥ is two characters long.
			Therefore for gid you would get:
			g
			gi
			gid
			gid(part of ¥)
			...
			The first section is the loop number faB 1
			The second is the field we are querying faB 2
			The third is the operator faB 3
			The forth is the value faB 4
			The fifth (if present) would be the other operator - i.e. AND/OR faB 5
			*/
			for ($fa=0;$fa<count($filterA);$fa++){
				//Is this a single loop?
				if ($faA == 1) {
					//Single loop
					if (strpos($$q, '¥') != false){
						//If there is a ¥ lets remove it
						str_replace("¥","",$$q);
						//Move to the next section of the string therefore $faB 1 is always blank (preceeding ¥)
						$faB = $faB + 1;
						$q = 'q' . $faB . $faC;
						//Does this section contain a ; character
						if ($filterA[$fa] == ';'){
							$$q = $$q . '~';
						} else {
							//Store the next section within the variable variable
							$$q = $$q . $filterA[$fa];
						}
					} elseif ($filterA[$fa] == ';') {
						$$q = $$q . '~';
					} else {
						//Store the section within the variable variable
						$$q = $$q . $filterA[$fa];
					}
				} else {
					//Multiple loops - Same as single loop but designed to cope with more data
					//Note that faB is the main loop i.e. ¥ position number and faC is the sub-loop
					if (strpos($$q, '¥') != false){
						str_replace("¥","",$$q);
						if ($faB == 4) {
							$faB = 1;
							...
							//lets remove the ' character which wraps the query
							$$q = str_replace("'","",$$q);
						} else {
							$faB = $faB + 1;
							if ($filterA[$fa] == ';'){
			...
			//Storing all the sub-loops (faC) into a variable filter2. 
			$filter2 = "";
			for ($fa=0;$fa<=$faC;$fa++){
				$q = 'q' . 2 . $fa;
				$filter2 = $filter2 . $$q;
				...
				if (strpos($$q, '=') > -1){
					//Possible need to use quotes
					$quoteIt = 1;
				} elseif (strpos($$q, 'LIKE') > -1) {
					//Possible need to use quotes
					$quoteIt = 2;
				} else {
					$quoteIt = 0;
				...
				if ($quoteIt == 1) {
					//We need to test the need for quotes, defining it based on numeric or not.
					$q = 'q' . 4 . $fa;
					if (is_numeric($$q)){
						$filter2 = $filter2 . $$q;
					} else {
					...
					//Definately need quotes for the LIKE query
					$q = 'q' . 4 . $fa;
					$filter2 = $filter2 . "'" . $$q . "'";
				} else {
					$q = 'q' . 4 . $fa;
...
switch($function):
	...
	Case 'view':
		...
		for ($tb=0;$tb<($tableLoopsno);$tb++) {
			...
			<span id=\"QPage1" . $tb . "\" class=\"showIt\">
				<table width=\"100%\"><tr>
				<td>
			...
			print "<table id='mainRow' class='mainRow'>\n";
			...
			do {
				...
				if ($ex==1){
					...
					if ($hideNow == 0) {
						print "<th id=\"T" . $tb . "col" . $ex2 . "\" class=\"hideIt\" style=\"padding:5px;font-weight:bold;\">";
					} else {
						print "<th id=\"T" . $tb . "col" . $ex2 . "\" class=\"hideIt\" style=\"padding:5px;font-weight:bold;\">";
					}
			...
			for ($i=0;$i<$rowNo;$i++){
				...
				do {
					...
					if ($j==0) {
						...
						print "<td headers=\"T" . $tb . "colH1\" style=\"padding:5px;\"><input type='button' id='backBut' value='<<' onclick='dynaScriptToggleCols(\"back\", " . $colNo . ", " . $maxCol . ", " . $tb . ")'  /></td>";
					}
