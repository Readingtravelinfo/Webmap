...
$(document).ready(function () {
    jQueryReady = true;
	//OK, this is my page loading event and from here we can call the required dev_functions as the page loads
...
function updateVariableStatusCall(listRef, dis){
	...
	workType = "'" + workType + "'";
	var servReq = "../../apps/dev_functions/vstatus.php?listid=NA&workType=" + workType;
...
function updateVariableStatus(listid, dis){
	...
	if (typeof listid != 'undefined'){
		var servReq = "../../apps/dev_functions/vstatus.php?listid=" + listid;
		$.get(servReq, function(data){
			...
			if (gidVal != null){
				...
				servReq = "../../apps/dev_functions/vstatus.php?listid=NA2&table=" + table + "&gid=" + gidVal;
				$.get(servReq, function(data){
...
function updateVariableIcon(listid, dis){
	var option;
	if (typeof listid != 'undefined'){
		var servReq = "../../apps/dev_functions/vstatus.php?listid=NA-L" + listid;
		$.get(servReq, function(data){
			//This ajax request will obtain the list of available status fields and we need to to split it up then update the status drop down boxes
			var statArr = data.split("|");
			...
			if (gidVal != null){
				//We need to get the record value from the server, one more post
				servReq = "../../apps/dev_functions/vstatus.php?listid=NA3&table=" + table + "&gid=" + gidVal;
				$.get(servReq, function(data){
					//This should be the current value
					var currVal = data;
					var foundit = 0;
	...
	if (usertext==="") {
		...
		if (okint == 0) {
			...
			if (table == 0 || orderBy == 0 || geomF == 0){
				...
				$.get("../../apps/dev_functions/tableset.php?table=" + table + "&fun=1", function(data){
					eval(data); //This is to cope with the fact we have no table definition at the start
					$.get("../../apps/dev_functions/tableset.php?table=" + table + "&fun=3&lookup=" + lookup, function(data){
						eval(data); //This is to cope with the fact we have no table definition at the start
						...
						if (filter.length == 0){
							queryString = "";
							tableHTML = "../../apps/dev_functions/tableedit.php";
		...
		if (xhr.readyState == 4 && xhr.status == 200)
		{
			if (!document.getElementById('Attributes Query')) {
				...
				if (tableHTML.indexOf("function=geomsave")!=-1) {
					...
					var textStr = "../../apps/dev_functions/dynamite.php" + tableHTML.substr(tmpLoc); //This removes the results.php section of the other URL
