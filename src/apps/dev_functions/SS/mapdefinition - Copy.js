//Global variables
var i, i2, centrePanel, mapPanel, legendPanel, tableport, viewport, tree, tableURL, accordion, fieldNames, currGID;
...
var BMapMini, geomOL, pointLayer, lineLayer, polygonLayer, simplePolygonLayer, selectedRow;
	...
	if (part1 == 1 || part2 == 1){
		...
		if (part2 == '&outputFormat=KML'){
			...
			$.post(url,function(data){
				...
				document.getElementById('ddupdate').innerHTML = '';
...
function loadmap() {
	...
	if (usertext==="") {
		...
		if (okint == 0) {
			...
			//We need to define any zooom to click events on the WFS layers
				if (overlayZoom[i]=='True' || overlayZoom[i]=='TRUE' || overlayZoom=='Yes' || overlayZoom=='YES'){
					tmpWFSnameCon = 'wfsControl' + i;
					window[tmpWFSnameCon] = new OpenLayers.Control.SelectFeature(window[tmpWFSname]);
				}
				if (overlayZoom[i]=='True' || overlayZoom[i]=='TRUE' || overlayZoom=='Yes' || overlayZoom=='YES'){
					tmpWFSnameCon = 'wfsControl' + i;
					map.addControl(window[tmpWFSnameCon]);
					window[tmpWFSnameCon].activate();
					window[tmpWFSname].events.on({
						featureselected: function(event) {
							alert("Yes");
							var tmpX, tmpY, tmpRef, tmpZL, tmpSF, tmpST;
							var CurrLayer = event.feature.fid.substring(0,event.feature.fid.indexOf("."));
							for (i2=0;i2<overlayArray.length;i2++){
								if (wfsFT[i2]==CurrLayer){
									tmpZL = overlayZoomLevel[i2];
									tmpSF = overlayZoomSelF[i2];
									tmpRT = overlayZoomRepT[i2];
									tmpRF = overlayZoomRepF[i2];
								}
							}
							tmpX = event.feature.geometry.x;
							tmpY = event.feature.geometry.y;
							if (tmpSF!='') {
								tmpRef = event.feature.data;
								tmpRef = tmpRef[tmpSF]; //Here we collect the reference field as required
								//We can not unselect the first selection and move to the filter job
								window[tmpWFSnameCon].unselectAll();
								//Now we filter select the WFS using the lookup and calculate the zoomto
