...
$dbconn = pg_connect($conn_string) or die('connection 1 failed' . pg_last_error());
$table = $_POST['table'];
...
$res = pg_query($dbconn, $query) or die('Query 1 failed: ' . pg_last_error());
...
if ($qtype == "INSERT") {
			...
			if (isset($val)) {
				...
				if ($i2 == 0) {
					...
					if ($i3 == 0) {
						...
						if($ft[$i]=='B') {
							if ($val == "on" || $val == "t" || $val == "true" || $val == "True" || $val == "yes") {
								$s2 = "true";
							} else {
								$s2 = "false";
							}
	...
	if ($i2 != -1) {
		...
		$bk = pg_query($dbconn, $query) or die('Query 2 failed: ' . pg_last_error());
		...
		$c = 0;
		while ($b < $bkcolNo) {
			//This creates the query string in the from (fieldname) values (value)
			...
			//If this is the first record we don't need a comma, otherwise we do
			if ($b==0) {
				$refStr2 = "";
				$refStr3 = "";
			}
			...
			//Most importantly, if the value is null we may need to exclude it.
			if ($type == 'bool') {
				if ($valStr == 'NULL') {
					$refStr2 .= $comStr . pg_field_name($bk, $b);
					$refStr3 .= $comStr . $valStr . "";
				} else {
					$refStr2 .= $comStr . pg_field_name($bk, $b);
					$refStr3 .= $comStr . "'" . $valStr . "'";
				}
			} elseif ($type=='geometry') {
				//Geometry changes are recorded by null values are ignored as these may cause errors, please delete from the database manually.
				if ($valStr != 'NULL') {
					$refStr2 .= $comStr . pg_field_name($bk, $b);
					$refStr3 .= $comStr . "'" . $valStr . "'";
				}
		...
		//Run the query
		$bk2 = pg_query($dbconn, $dynqstr2) or die('Query 3 failed: ' . pg_last_error());
		pg_free_result($bk);
		pg_free_result($bk2);
	} else {
		//At this stage this represents an error
	...
	if (substr($table, 0,2) != 'ue') {
		//Download the existing record
		$query = 'SELECT * FROM ' . pg_escape_identifier($table) . ' WHERE gid = ' . $gidVal . ';';
		$bk = pg_query($dbconn, $query) or die('Query 4 failed: ' . pg_last_error());
		...
		//Run the query
		$bk2 = pg_query($dbconn, $dynqstr2) or die('Query 5 failed: ' . pg_last_error());
		pg_free_result($bk);
		pg_free_result($bk2);
	}
...
//$debugging .= " " . $dynqstr;
if ($eerr < 1){
	$res2 = pg_query($dbconn, $dynqstr) or $result = pg_last_error() . " Save function ";
} else {
