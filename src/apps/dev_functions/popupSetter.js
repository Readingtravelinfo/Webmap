...
function hoverHTML(features) {
	...
	//OK, for hovertext we want to remove duplicates, lets create duplicate array to use later
	var duplicateArray = [];
	var fi, fi2, dupCatch;
	...
	for (fi=0;fi<features.length; fi++){
		...
		if (features.length==1) {
			...
			if (fi==0) {
				...
				htmlText2 += templateText;
				duplicateArray.push(templateText);
				...
				dupCatch = 0;
				for (fi2=0;fi2<duplicateArray.length; fi2++){
					if(duplicateArray[fi2]==templateText){
						//Duplicate found
						dupCatch = 1;
					} 
				}
