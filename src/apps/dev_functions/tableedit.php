...
//dev_functions to exclude
$functionEx = array();
foreach ($xml->dev_functions->functionEx as $opt) {
...
switch($function):
	Case 'edit':
        ...
        for ($i=0;$i<$specialno;$i++){
            if ($table==$specialTables[$i]){
                print "<img src=\"../../apps/dev_functions/blank.jpg\" onLoad=\"specialT('" . $table . "','edit','" . $gid . "')\" />";
            }
        }
			...
			if ($ex==1){
				...
				if ($sel==1){
					...
					if ($fnarray[$j] == 'worksstatus'){
						print "<img src='../../apps/dev_functions/blank.jpg' style='display:none' onLoad='updateVariableStatusCall(\"Sworkstype\",1)' />";
					}
					print "</td></tr>\n";
				} elseif ($sel==2){
					//Lookup List
				...
				if ($sel==1){
					...
					print "</select>";
					if ($fnarray[$j] == 'worksstatus'){
						print "<img src='../../apps/dev_functions/blank.jpg' style='display:none' onLoad='updateVariableStatusCall(\"Sworkstype\",0)' />";
					}
					print "</td></tr>\n";
				} elseif ($sel==2){
					//Lookup List
		...
		print "</table>\n";
		if ($tableGeomEditVal=='Yes'){
			//print "<img src=\"../../apps/dev_functions/blank.jpg\" onload='editMap(\"" . pg_fetch_result($res2,0,0) . "\")' />\n";
		}
		print "</div>";
	...
	Case 'add':
        ...
        for ($i=0;$i<$specialno;$i++){
            if ($table==$specialTables[$i]){
                print "<img src=\"../../apps/dev_functions/blank.jpg\" onLoad=\"specialT('" . $table . "','Add','*')\" />";
            }
        }
	...
	Case 'view':
		for ($i=0;$i<$specialno;$i++){
            if ($table==$specialTables[$i]){
                print "<img src=\"../../apps/dev_functions/blank.jpg\" onLoad=\"specialT('" . $table . "','view','" . $gid . "')\" />";
            }
        }
		...
		for ($tb=0;$tb<($tableLoopsno);$tb++) {
			...
			$res = pg_query($dbconn, $query) or header( print "<img src=\"../../apps/dev_functions/blank.jpg\" onLoad=\"resetView('" . $table . "', " . $tb . ", 0)\" />");
			...
			print "
				...
				print "</td><td>
				";
					//<input type=\"button\" value=\"Return to Map\" onclick=\"return2Map('collapse', '" . $table . "')\" />"; No Return dev_functions here
		...
		print "<img src='../../apps/dev_functions/blank.jpg' onload=\"dynaToggleCols(" . $tableLoopsno . ", '" .  $colNoStr . "')\" />
		";
		break;
	...
	case 'geomsave':
		...
		break;
	case 'blank':
		print "<div id='holdscreen'><img src='../../apps/dev_functions/loader.gif' /><br />
		...
		break;
	default:
		print "<img src='../../apps/dev_functions/blank.jpg' onload=\"resetView('" . $table . "', 0, 0)\" />";
		break;
endswitch;
